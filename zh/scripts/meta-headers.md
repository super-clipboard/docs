...
> **关于 `@namespace`**：从 v0.5 起脚本身份完全由安装来源 URL 派生
> （`updateURL` / `downloadURL` / 市场链接），KV 与 `scriptData` 都按这个
> 派生出的 *script id* 隔离，**与 `@namespace` 无关**。`@namespace` 现在
> 只作为可选的展示元数据保留，写不写都不影响功能；仍建议填一个反向域名
> 风格的字符串方便人类识别。
...
| 字段 | 默认 | 取值 | 说明 |
|------|------|------|------|
| `@namespace` | — | 反向域名风格字符串 | 仅作展示/识别用，不再用于隔离 |
